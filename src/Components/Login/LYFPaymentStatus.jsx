...
export default function LYFPaymentStatus() {
  ...
  if (JSON.parse(jsonData)?.body?.resultInfo?.resultStatus == "TXN_FAILURE") {
    ...
    const data1 = JSON.parse(sessionStorage.getItem("demoYatraF"));
    const data2 = JSON.parse(sessionStorage.getItem("demoMailYatraF"));
    if (Object.keys(jsonData).length && data1 && data2) {
      const placeorder = async () => {
        let user = JSON.parse(agentDetails);

        sessionStorage.removeItem("demoYatraF");
        sessionStorage.removeItem("demoMailYatraF");
          ...
          <div>
            ...
            <div className="d-flex justify-content-center">
              <p style={{ fontSize: "18px" }}>
                Flight Tickets are successfully Booked...
              </p>
            </div>
            <br />
            <div className="d-flex justify-content-center">
